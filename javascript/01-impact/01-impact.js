// A. Reference Ids:
// Bar graph Element:
var barGraphCountyVizElement = document.getElementById("bar_graph_county_viz");		// Data will be shown here
var regionPieChartVizElement = document.getElementById("pie_chart_region_viz");	
var barGraph92CountiesVizElement = document.getElementById("bar_graph_all_92_viz");	

var regionMeanElement = document.getElementById("region_mean_value");	
var regionMedianElement = document.getElementById("region_median_value");	
var regionModeElement = document.getElementById("region_mode_value");	

// B. Global variables:
var impactData = [];			// Store the impact data
var county_selected = null;		// Keep a track of the current county clicked on

// Reference to the bar graph
var barReferences;
var all92CountiesBarReferences;

var min_impact_value = 0;								// Store the minimum value for the visio data
var max_impact_value = 0;								// Store the maximum value for the visio data

var total_E_and_W_activities = 0;						// Total Education and Workforce activity count
var total_Technology_Adoption = 0;						// Total Technology Adoption activity count
var activities_count_array = [];


// C. create the data structure to store the data:
// Custom (hard-coded) data structure to store the values:
// Store the count for the individual counties:
var barGraphDataset = [
	{"Lake" : 0, "Porter" : 0, "LaPorte" : 0, "Starke" : 0, "Newton" : 0, "Jasper" : 0, "Pulaski" : 0},																					// Region 1 at index 0
	{"StJoseph" : 0, "Elkhart" : 0, "Marshall" : 0, "Kosciusko" : 0, "Fulton" : 0},																										// Region 2
	{"LaGrange" : 0, "Steuben" : 0, "Noble" : 0, "DeKalb" : 0, "Whitley" : 0, "Allen" : 0, "Wabash" : 0, "Huntington" : 0, "Wells" : 0, "Adams" : 0, "Grant" : 0},						// Region 3
	{"White" : 0, "Cass" : 0, "Miami" : 0, "Carroll" : 0, "Howard" : 0, "Tipton" : 0, "Clinton" : 0, "Tippecanoe" : 0, "Benton" : 0, "Warren" : 0, "Fountain" : 0, "Montgomery" : 0},	// Region 4
	{"Boone" : 0, "Hamilton" : 0, "Madison" : 0, "Hancock" : 0, "Shelby" : 0, "Johnson" : 0, "Morgan" : 0, "Hendricks" : 0},															// Region 5
	{"Blackford" : 0, "Jay" : 0, "Randolph" : 0, "Delaware" : 0, "Henry" : 0, "Wayne" : 0, "Rush" : 0, "Fayette" : 0, "Union" : 0},														// Region 6
	{"Vermillion" : 0, "Parke" : 0, "Putnam" : 0, "Vigo" : 0, "Clay" : 0, "Sullivan" : 0},																								// Region 7
	{"Owen" : 0, "Greene" : 0, "Monroe" : 0, "Brown" : 0, "Lawrence" : 0, "Daviess" : 0, "Martin" : 0, "Orange" : 0},																	// Region 8
	{"Franklin" : 0, "Decatur" : 0, "Bartholomew" : 0, "Jackson" : 0, "Jennings" : 0, "Ripley" : 0, "Dearborn" : 0, "Ohio" : 0, "Switzerland" : 0, "Jefferson" : 0},					// Region 9
	{"Scott" : 0, "Washington" : 0, "Clark" : 0, "Floyd" : 0, "Crawford" : 0, "Harrison" : 0},																							// Region 10
	{"Knox" : 0, "Gibson" : 0, "Pike" : 0, "Dubois" : 0, "Posey" : 0, "Vanderburgh" : 0, "Warrick" : 0, "Spencer" : 0, "Perry" : 0},													// Region 11
	{"Marion" : 0}																																										// Region 12
	];


// // Works but throws an error - this.C is undefined:
$('#infoVideoModal').on('hidden.bs.modal', function () {
    var src = $(this).find('iframe').attr('src');
    $(this).find('iframe').attr('src', '');
    $(this).find('iframe').attr('src', src);
});

// D. Create a tooltip to show data when you hover:
var tooltip = d3.select("body").append("div")
	.attr("class", "tooltip")
	.style("opacity", 0);

var tooltipWidth = 80;
var tooltipHeight = 35;

// E. Create propoerties of other graphs and elements in the page:
// Single region bar graph:
var barGraphCountyMargin = { top:30, right:10, bottom:30, left:50 }; 
var countyBarGraphProperties = {
	id : "bar_graph_county_viz",
	margin : barGraphCountyMargin,
	width : barGraphCountyVizElement.clientWidth - barGraphCountyMargin.left - barGraphCountyMargin.right,
	height : 300 								 - barGraphCountyMargin.top  - barGraphCountyMargin.bottom,
	barPadding : 2,
	chartPadding : 20,
	xLabel : "County Name",
	yLabel : "IN-MaC impact value",
	dataset : null,						// Pass data here if it is a static bar chart
	updatable : false,
	heightPadding : 150,
	xLabelPadding : 20,
};

// All counties bar graph:
var barGraphAll92CountiesMargin = { top:30, right:10, bottom:30, left:50 }; 
var all92CountiesBarGraphProperties = {
	id : "bar_graph_all_92_viz",
	margin : barGraphAll92CountiesMargin,
	width : barGraph92CountiesVizElement.clientWidth - barGraphAll92CountiesMargin.left - barGraphAll92CountiesMargin.right,
	height : 300 								 - barGraphAll92CountiesMargin.top  - barGraphAll92CountiesMargin.bottom,
	barPadding : 2,
	chartPadding : 20,
	xLabel : "County Name",
	yLabel : "IN-MaC impact value",
	dataset : null,			// Pass data here if it is a static bar chart
	updatable : false,
	class : "county",
	heightPadding : 5,
	xLabelPadding : 20,
	fontSize : "7px",
};

// Pie chart for all regions:
var regionPieChartMargin = { top:15, right:5, bottom:5, left:5 }; 
var regionPieChartWidth = regionPieChartVizElement.clientWidth - regionPieChartMargin.left - regionPieChartMargin.right;
var regionPieChartHeight = 200 						 		   - regionPieChartMargin.top  - regionPieChartMargin.bottom;

var countyPieChartProperties = {
	id : "pie_chart_region_viz",
	margin : regionPieChartMargin,
	width : regionPieChartWidth,
	height : regionPieChartHeight,
	radius : Math.min(regionPieChartWidth, regionPieChartHeight) / 2 - regionPieChartMargin.top,
	dataset : null,			// Pass data here if it is a static bar chart
	updatable : false,
	heightPadding : 5,
	tooltipData : "Total IN-Mac impact in the region",
	tooltipHeight : "50px",
};

// a. Indiana Map visualization Parameters:
var mapMargin = { left:70, right:40, top:60, bottom:40 };

// Width and height of the layout:
var width  = 	700 - mapMargin.left - mapMargin.right;
var height = 	800 - mapMargin.top  - mapMargin.bottom;

// Add an SVG to draw the map on::  
var mapSvg = d3.select("#map_viz")
	.append("svg")
	.attr("id", "mapSvg")
    	.attr("viewBox", function(){
    		return ("0 0 " + (width  + mapMargin.left + mapMargin.right).toString() + " " 
    					   + (height + mapMargin.top  + mapMargin.bottom).toString());
    	})
    	.attr("width", "100%")
    	.attr("height", "100%")
    	.attr("class", "element_boundary");

var mapGroup = mapSvg.append("g")
	        		.attr("transform", "rotate(6)");

// F. Create the map visualization:
// Create a d3 projection:
var projection = d3.geoAlbersUsa()
	.scale(11000)
	.translate([-1150, 640]);

// Use the projects for the path:
var path = d3.geoPath()
	.projection(projection);

// Colors for the choropleth map:
// var colorScale = d3.scaleQuantile()
// 					.range(["#FFF9C4", "#FFF176", "#FFEB3B", "#FBC02D", "#F57F17"])		// Shades of yellow

// Init function call:
function init()
{
	// E. Load the data and extract the information we need:
	d3.csv("data/01-impact_data.csv").then(function(data)
	{
		// console.log(data);
		var i = 0;
		data.forEach(function(d){
			impactData.push([d.County]);								// Extract the County name
			impactData[i].push(+d.E_and_WD_Activities);					// Extract the E and WD activities count
			impactData[i].push(+d.Technology_Adoption);					// Extract the TA Sites Reserved count
			impactData[i].push(d.Region_ID);							// Append the region ID as the last element
			impactData[i].push(impactData[i][1] + impactData[i][2]);	// Add all the values and store here

			total_E_and_W_activities += impactData[i][1];
			total_Technology_Adoption += impactData[i][2];

			// Calculate the minimum and maximum value:
			if(impactData[i][4] < min_impact_value)
				min_impact_value = impactData[i][4];
			if(impactData[i][4] > max_impact_value)
				max_impact_value = impactData[i][4];
			
			// Increment the data structure that stores the count of the total microgrants awarded in the county:
			for(var j = 0; j < 12; j++)
			{
				if(j+1 == +d.Region_ID)
					barGraphDataset[j][d.County] += impactData[i][4];
			}
			i++;

		});

		activities_count_array[0] = total_Technology_Adoption;
		activities_count_array[1] = total_E_and_W_activities;

		// Since we want to show white for no value at all, we have the min value + 1 as the lowest yellow color:
		// colorScale.domain([min_impact_value+1, max_impact_value]);

		// CREATE SINGLE REGION (COUNTY) BAR GRAPH:
		// Compute the data to be passed to the bar graph to draw:
		countyBarGraphProperties.dataset = generateSingleCountyData("Lake", 1, barGraphDataset);
		countyBarGraphProperties.updatable = true;
		barReferences = createBarGraph(countyBarGraphProperties);
		// On page load call here since this is the last thing that will be called:
		updateBarGraph(barReferences, "Lake", "1");				// Show bar graph

		countyPieChartProperties.dataset = generateRegionData(barGraphDataset);
		createPieChart(countyPieChartProperties);
		pieOut("1");
		createRegionStats(countyPieChartProperties.dataset);

		// CREATE ALL 92 COUNTIES BAR GRAPH:
		all92CountiesBarGraphProperties.dataset = generate92CountiesData(barGraphDataset);
		all92CountiesBarGraphProperties.updatable = false;
		all92CountiesBarReferences = createBarGraph(all92CountiesBarGraphProperties);

	 	createBarGraph92CountiesLegend(all92CountiesBarReferences);

	});
}

// Init:
init();

// Load the map data:
d3.json("data/indiana_counties.json").then(function(indianaData) {

	for(var i = 0; i < impactData.length; i++)
	{
		var county = impactData[i][0];
		var E_and_WD_Activities_count = impactData[i][1];
		var Technology_Adoption_count = impactData[i][2];
		var total_activity_count = impactData[i][4];
		for(var j = 0; j < (topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features).length; j++)
		{
			if(county == topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features[j].properties.NAME)
			{
				topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features[j].properties.E_and_WD_Activities = E_and_WD_Activities_count;
				topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features[j].properties.Technology_Adoption = Technology_Adoption_count;
				topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features[j].properties.impact_data = total_activity_count;
				break;
			}
		}
	}

	// Create a color scale for the values and the radius:
	// The range values are the different shades of red:
	var color_0_500 = d3.scaleQuantile()
						.domain([1, 500])
						.range(["#FF0000"]);

	var radius_0_500 = d3.scaleSqrt()
						.domain([1, 500])
						.range([10, 40]);

	var color_500_1000 = d3.scaleQuantile()
						.domain([501, 1000])
						.range(["#00FF00"]);

	var radius_500_1000 = d3.scaleSqrt()
						.domain([501, 1000])
						.range([10, 40]);

	var color_1000_1500 = d3.scaleQuantile()
						.domain([1001, 1500])
						.range(["#0000FF"]);

	var radius_1000_1500 = d3.scaleSqrt()
						.domain([1001, 1500])
						.range([10, 40]);

	var color_1500_greater = d3.scaleQuantile()
						.domain([1501, max_impact_value])
						.range(["#000000"])

	var radius_1500_greater = d3.scaleSqrt()
						.domain([1501, max_impact_value])
						.range([10, 40]);

	// Append all the counties:
	mapGroup.append("g")
			.attr("class", "counties")					// Add the counties class attribute
			.selectAll("path")
				.data(topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features)
				.enter()
				.append("path")
					.attr("d", path)
					// Use this if white doesn't work, or try with different users
					.attr("fill", defualtCountyColor)
					.attr("fill-opacity", 0.4)
					.attr("id", function(d){ return d.properties.NAME;})

	// Create the bubbles:
	var bubble_group = mapGroup.append("g")
			.selectAll("circle")
			.data(topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features)
			.enter()
			.append("circle")
			.attr("transform", function(d){
				return "translate(" + path.centroid(d) + ") rotate(-6)";
			})
			.attr("r", function(d){
				if(d.properties.impact_data >= 1 && d.properties.impact_data < 500){
					return radius_0_500(d.properties.impact_data);
				}
				else if(d.properties.impact_data >= 501 && d.properties.impact_data < 1000){
					return radius_500_1000(d.properties.impact_data);
				}
				else if(d.properties.impact_data >= 1001 && d.properties.impact_data < 1500){
					return radius_1000_1500(d.properties.impact_data);
				}
				else if(d.properties.impact_data >= 1501 && d.properties.impact_data < max_impact_value+1){
					return radius_1500_greater(d.properties.impact_data);
				}
			})
			.style("fill", function(d){
				if(d.properties.impact_data >= 1 && d.properties.impact_data < 500)
					return color_0_500(d.properties.impact_data);
				else if(d.properties.impact_data >= 501 && d.properties.impact_data < 1000)
					return color_500_1000(d.properties.impact_data);
				else if(d.properties.impact_data >= 1001 && d.properties.impact_data < 1500)
					return color_1000_1500(d.properties.impact_data);
				else if(d.properties.impact_data >= 1501 && d.properties.impact_data < max_impact_value+1)
					return color_1500_greater(d.properties.impact_data);				
			})
			.attr("fill-opacity", "0.3")
			.attr("stroke", "black")
			.style("stroke-opacity", "1.0")
			.attr("stroke-width", "1.0");

	// Add the borders to the visualization as per the regions:
	appendBorder(indianaData);

	// Add actual values, but make the fill to be transparent to view the underlying colors:
	var new_group = mapGroup.append("g")
		.attr("class", "states")		// Color the states to light blue
	.selectAll("path")
		.data(topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features)
	.enter().append("path")
		.attr("class", "county")		
		.attr("d", path)
		// Set default colors to the counties at initiation:
		.style("fill", function(d){
			return "transparent";})
		.attr("id", function(d){
			return d.properties.NAME;
		})
		// Show Visio data on mouse click:
		.on("click", function(d){	
			county_selected = d.properties.NAME;
			barReferences.dataset = generateSingleCountyData(d.properties.NAME, d.properties.REGION_ID, barGraphDataset);

			// Show information through the drop down and the bar graph:
			updateBarGraph(barReferences, d.properties.NAME, d.properties.REGION_ID);

			pieOut(d.properties.REGION_ID);
		})
		.on("mouseover", function(d){
			d3.select(this).style("cursor", "pointer");
		})
		.on("mouseout", function(d){
			d3.select(this).style("cursor", "default");
		});

	// Add text to show all the county names:
	var textGroup = mapGroup.selectAll("text")
			.data(topojson.feature(indianaData, indianaData.objects.cb_2015_indiana_county_20m).features)
			.enter()
			.append("text")
			.attr("transform", function(d){
				return "translate(" + path.centroid(d) + ") rotate(-6)";
			})
			.style("fill", "black")
			.attr("font-family", "sans-serif")
			.attr("text-anchor", "middle")
			.attr("font-size", "9px")
			.text(function(d){
				return d.properties.NAME;
			})

			// Increase the font-size on mouseover:
			.on("mouseover", function(d){
				d3.select(this).style("cursor", "pointer");

				var county_name = d.properties.NAME;
				var E_and_WD_Activities = d.properties.E_and_WD_Activities;
				var Technology_Adoption = d.properties.Technology_Adoption;

				tooltip.transition()
					.duration(200)
					.style("opacity", 1.0)
					.style("width", function(){
						tooltipWidth = 200;
						return tooltipWidth + "px";
					});

				// Set the name to show on hover in the tooltip
				tooltip.html(
					"<strong>Region: </strong>" + d.properties.REGION_ID + "<br>" +
					"<strong>County: </strong>" + county_name + "<br>" +
					"<strong>Technology Adoption: </strong>" + Technology_Adoption + "<br>" +
					"<strong>E & WD Activities: </strong>" + E_and_WD_Activities  
					)
				.style("height", function(){
					tooltipHeight = 65;
					return tooltipHeight + "px";
				})
				.style("left", (d3.event.pageX - tooltipWidth / 2) + "px")
				.style("top", (d3.event.pageY + 20) + "px");

				d3.select(this)
					.attr("font-weight", "bold")
					.attr("font-size", "15px")
			})

			// Make the font-size normal on mouseout:
			.on("mouseout", function(d){
				d3.select(this)
					.attr("font-weight", "normal")
					.attr("font-size", "9px")
					.style("cursor", "default");

				tooltip.transition()
					.duration(500)
					.style("opacity", 0)
			})
			// Show Visio data on mouse click:
			.on("click", function(d){	
				county_selected = d.properties.NAME;
				barReferences.dataset = generateSingleCountyData(d.properties.NAME, d.properties.REGION_ID, barGraphDataset);

				// Show information through the drop down and the bar graph:
				updateBarGraph(barReferences, d.properties.NAME, d.properties.REGION_ID);

				pieOut(d.properties.REGION_ID);
			});

		// Create the legend for the impact values:
		createImpactLegend();

});

// Stores the URIs of all SVG elements:
var allSVGUri = [];
// Store all images:
var all_imgs = [];

// Wait for inner functions to finish:
async function generateReport()
{
	// Reset the values:
	allSVGUri = [];
	all_imgs = [];

	// Finish these functions:
	await SVGUri("#mapSvg");
	await SVGUri("#countySVG");
	await SVGUri("#all92CountiesSVG")
	await SVGUri("#pieSVG")
	// console.log(allSVGUri);

	for(var i = 0; i < allSVGUri.length; i++){
		// Wait for this function to finish:
		await convertURIToImageData(allSVGUri[i]).then(function(imageData){
			// console.log("7: Finished convertURIToImageData called")

		});
	}

	// await createPDF();
	await generatePDF();
}

function generatePDF()
{
	var doc = new jsPDF();
	// 'a4'  : [ 595.28,  841.89]
	// A4 measures 210 × 297 millimeters or 8.27 × 11.69 inches.
	// Add title:
	
	var mapWidth = width;
	var mapHeight = height;
	var countyBarGraphWidth = countyBarGraphProperties.width;
	var countyBarGraphHeight = countyBarGraphProperties.height;
	var all92CountiesBarGraphWidth = all92CountiesBarGraphProperties.width;
	var all92CountiesBarGraphHeight = all92CountiesBarGraphProperties.height;
	var pieChartWidth = countyPieChartProperties.width;
	var pieChartHeight = countyPieChartProperties.height;

	var maxWidth_1 = 90;
	var maxWidth_2 = 190;

	//A. Create the title:
	doc.setFontSize(12);
	doc.setFont("times");
	doc.setFontStyle("bold");
	doc.text(80, 10, 'IN-MaC Impact Report');
	
	// B. Create the date and time stamp:
	doc.setFontSize(6);
	doc.setFontType("italic");
	var options = { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', 
					hour: 'numeric', minute: 'numeric', second: 'numeric' };
	var reportGenerationTime = Intl.DateTimeFormat('en-US', options).format(new Date());

	//var reportGenerationTime = "Created on: " + new Date();
	doc.text(90, 14, reportGenerationTime);

	// C. Add the In-MaC logo:
	var inMacLogo = "data:image/png;base64,";
	doc.addImage(inMacLogo, 'png', 160, 3, 40, 15);


	// D. Add the map and related information:
	doc.setFontType("normal");
	doc.setFontSize(8);
	var mapText = "Overview: IN-MaC creates a stronger, more competitive manufacturing ecosystem " + 
	"for Indiana and the nation. IN-MaC does so by mobilizing its resources, expertise and network to" + 
	" strengthen the relationship between workforce education, innovation, smart manufacturing, and " + 
	"research to elevate Indiana as the manufacturing destination.";
	doc.text(10, 25, mapText, {'maxWidth' : maxWidth_2});

	var inmacEcosystemText = "IN-MaC Ecosystem: IN-MaC ecosystem connects workforce education " + 
	"activities across the state of Indiana. The essence of a workforce education ecosystem is to " + 
	"establish collaboration that allows each entity to interact successfully. Ecosystems come alive " + 
	"by connecting all the elements that are working in the same space. Creating intersections to " + 
	"facilitate industry, education, ideas, and resources.";
	doc.text(10, 37, inmacEcosystemText, {'maxWidth' : maxWidth_2});

	var ecosystemText = "Ecosystem: The ecosystems allows for a fast flow of information and " + 
	"resources to help those within workforce education and industry to quickly identify like work " + 
	"and developing strategy focused on workforce education. Highlight and sharing a vested interest " + 
	"in workforce education and industry across the state of Indiana. An approach to support meaningful " + 
	"change for disconnected entities. An ecosystem for educators, students, industry, and communities " + 
	"to create meaningful learning experiences and partnerships.";
	doc.text(10, 50, ecosystemText, {'maxWidth' : maxWidth_2});

	var base = 70;

	// E. Map image:
	doc.setFontType("bold");
	var mapTitleText = "IN-MaC's impact in Workforce and Education over the entire state of Indiana";
	doc.text(10, base, mapTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[0], 'JPEG', 1, base + 5, 120 * mapWidth / mapHeight, 120 );
	

	// F. County bar graph:
	var countyBarGraphTitleText = "Distribution of impact of IN-MaC in a specific Economic Growth Region";
	doc.text(110, base, countyBarGraphTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[1], 'JPEG', (120 * mapWidth / mapHeight) + 10, base + 5,
										45 * countyBarGraphWidth / countyBarGraphHeight, 
										45 );


	// G. Pie Chart:
	var pieChartTitleText = "Percent impact of IN-MaC across Indiana's 12 EGR";
	doc.text(110, 45 * countyBarGraphWidth / countyBarGraphHeight + base + 5,
			pieChartTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[3], 'JPEG', (120 * mapWidth / mapHeight) + 10, 
						  				45 * countyBarGraphWidth / countyBarGraphHeight + base + 5,
									  40 * pieChartWidth / pieChartHeight, 40 );

	// H: Stats field:
	var statsTitleText = "Statistics for the impact in each Economic Growth Region";
	doc.text(110, 45 * countyBarGraphWidth / countyBarGraphHeight + base + 50,
		statsTitleText, {'maxWidth' : maxWidth_1});
	doc.setFontSize(12);
	var statsMeanText = "Region Mean : " + d3.format(".2s")(statsData.mean); 
	var statsMedianText = "Region Median : " + d3.format(".2s")(statsData.median);
	var statsModeText = "Region Mode : " + statsData.mode.join(", ");
	doc.text(110, 45 * countyBarGraphWidth / countyBarGraphHeight + base + 55,
			statsMeanText, {'maxWidth' : maxWidth_1});
	doc.text(110, 45 * countyBarGraphWidth / countyBarGraphHeight + base + 60,
			statsMedianText, {'maxWidth' : maxWidth_1});
	doc.text(110, 45 * countyBarGraphWidth / countyBarGraphHeight + base + 65,
			statsModeText, {'maxWidth' : maxWidth_1});

	var baseDistance = 140;
	var textDistance = 75;
	// I: All 92 county bar graph:
	doc.setFontSize(8);
	var all92CountyBarGraphTitleText = "Comparison of total impact of IN-MaC in various counties spread in different regions over the entire state of Indiana.";
	doc.text(10, baseDistance + base - 5, all92CountyBarGraphTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[2], 'JPEG', 10, baseDistance + base + 5,
										35 * all92CountiesBarGraphWidth / all92CountiesBarGraphHeight, 
										35 );

	var field_1 = document.getElementById("field_1");													// Set its ID
	var field_2 = document.getElementById("field_2");													// Set its ID
	var field_3 = document.getElementById("field_3");													// Set its ID

	var field_1_text = "IMPACTED: " + field_1.innerText;
	var field_2_text = "REGIONS IMPACTED: " + field_2.innerText;
	var field_3_text = "COUNTIES IMPACTED: " + field_3.innerText;
	doc.setFontSize(10);
	doc.text(170, base + 15, field_1_text, {'maxWidth': 30});
	doc.text(170, base + 23, field_2_text, {'maxWidth': 30});
	doc.text(170, base + 35, field_3_text, {'maxWidth': 30});

	doc.save("IN-MaC Impact Report.pdf")

}