// A. Reference Ids:
// Line graph Element:
var lineGraphCountyVizElement = document.getElementById("line_graph_county_viz");		// Data will be shown here
...
var infoCardTextElement 	= document.getElementById("info_card_text");

// B. Global variables:
var schoolAttendanceData = [];
...
var dataForReport = ["County", "Corporation Name", "School Name", "School ID", "Attendance"];

// E. Create propoerties of other graphs and elements in the page:
// Single region bar graph:
var lineGraphCountyMargin = { top:30, right:10, bottom:30, left:45 }; 
var lineGraphProperties = {
	id : "line_graph_county_viz",
	margin : lineGraphCountyMargin,
	width : lineGraphCountyVizElement.clientWidth - lineGraphCountyMargin.left - lineGraphCountyMargin.right,
	height : 300 								  - lineGraphCountyMargin.top  - lineGraphCountyMargin.bottom,
...
function init()
{
	...
	///////
	d3.csv("../data/11-school_attendance_data.csv").then(function(data)
	{
		// console.log(data);
		var i = 0;
		data.forEach(function(d){
			...
			var data = {
				"Region" : Object.entries(d)[0][1],
				"County" : Object.entries(d)[1][1],
				"Corporation ID" : Object.entries(d)[2][1],
				"Corporation Name" : Object.entries(d)[3][1],
				"School ID" : Object.entries(d)[4][1],
				"School Name" : Object.entries(d)[5][1],
				"2017-18" : parseFloat(Object.entries(d)[6][1].replace('%', '')),
				"2016-17" : parseFloat(Object.entries(d)[7][1].replace('%', '')),
				"2015-16" : parseFloat(Object.entries(d)[8][1].replace('%', '')),
				"2014-15" : parseFloat(Object.entries(d)[9][1].replace('%', '')),
				"2013-14" : parseFloat(Object.entries(d)[10][1].replace('%', '')),
				"2012-13" : parseFloat(Object.entries(d)[11][1].replace('%', '')),
				"2011-12" : parseFloat(Object.entries(d)[12][1].replace('%', '')),
				"2010-11" : parseFloat(Object.entries(d)[13][1].replace('%', '')),
				"2009-10" : parseFloat(Object.entries(d)[14][1].replace('%', '')),
				"2008-09" : parseFloat(Object.entries(d)[15][1].replace('%', '')),
				"2007-08" : parseFloat(Object.entries(d)[16][1].replace('%', '')),
				"2006-07" : parseFloat(Object.entries(d)[17][1].replace('%', '')),
				"2005-06" : parseFloat(Object.entries(d)[18][1].replace('%', '')),
			};

			// Push the data:
			schoolAttendanceData.push(data);
		});

		// CREATE SINGLE REGION (COUNTY) LINE GRAPH:
		showDropDownData("Lake", "1", "schoolAttendance", schoolAttendanceData);			// Show the school data
...
d3.json("../data/indiana_counties.json").then(function(indianaData) {
	...
	mapGroup.append("g")
			...
			.on("click", function(d){
				...
				showDropDownData(d.properties.NAME, d.properties.REGION_ID, "schoolAttendance", schoolAttendanceData);
...
function generatePDF()
{
	...
	var lineGraphWidth = lineGraphProperties.width;
	var lineGraphHeight = lineGraphProperties.height;
	...
	doc.text(65, 10, 'Indiana School Attendance Data Report');
	...
	doc.setFontSize(8);
	var mapText = "The following dataset provides information on the attendance rates of various schools" + 
	" across the state of Indiana over a period of a few years.";
	doc.text(10, 25, mapText, {'maxWidth' : maxWidth_2});

	// E. Map image:
	doc.setFontType("bold");
	var mapTitleText = "School's in each county based on the 12 Indiana Economic Growth regions";
	doc.text(10, 35, mapTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[0], 'JPEG', 1, 40, 120 * mapWidth / mapHeight, 120 );
	

	// F. County bar graph:
	var lineChartTitleText = "Line chart to show the attendance trend of the selected school in a specific Economic Growth Region";
	doc.text(110, 35, lineChartTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[1], 'JPEG', (120 * mapWidth / mapHeight) + 10, 40,
										40 * lineGraphWidth / lineGraphHeight, 
										40 );
	...
	// County:
	doc.text(110, baseDistance + textDistance, "County: " + dataForReport[0]);

	// Organization:
	doc.text(110, baseDistance + textDistance + 5, "Corporation Name: " + dataForReport[1]);

	// Contact Person:
	doc.text(110, baseDistance + textDistance + 10, "School Name: " + dataForReport[2]);

	// Email:
	doc.text(110, baseDistance + textDistance + 15, "School ID: " + dataForReport[3]);

	// Website:
	doc.text(110, baseDistance + textDistance + 20, "Attendance Rates: ");
	//doc.text(10, baseDistance + textDistance + 20, "Attendance Rates: " + dataForReport[4]);
	for(var i = 0; i < 13; i++)
	{
		doc.text(110, baseDistance + textDistance + 25 + (i * 3), allYears[i] + ": " + dataForReport[4][i], {'maxWidth' : maxWidth_1});
	}

	// Save the report:
	doc.save("School Attendance Data Report.pdf")
