...
// B. Global variables:
var principalData = [];				// Store the principal data
...
var countyPieChartProperties = {
	...
	updatable : false,
	heightPadding : 5,
	tooltipData : "Total schools in the region",
	tooltipHeight : "50px",
};
...
function init()
{
	...
	///////
	d3.csv("../data/03-school_principal_data.csv").then(function(data)
	{
		var i = 0;
		data.forEach(function(d){
			principalData.push([d.COUNTY_NAME]);											// Extract the County name
			principalData[i].push(d.SCHOOL_NAME);											// Extract the School name
			principalData[i].push(d.PRINCIPAL_FIRST_NAME + " " + d.PRINCIPAL_LAST_NAME);	// Extract the Contact name
			principalData[i].push(d.CORPORATION_NAME);										// Extract the Corporation name
			principalData[i].push(d.ADDRESS + ", " + d.CITY + ", " + d.ZIP);				// Extract the Contact Address
			principalData[i].push(d.SCHOOL_HOMEPAGE);										// Extract the Webpage
			principalData[i].push(d.REGION_ID);
			i++;
			// Increment the data structure that stores the count of the total microgrants awarded in the county:
			for(var j = 0; j < 12; j++)
			{
				if(j+1 == +d.REGION_ID)
					barGraphDataset[j][d.COUNTY_NAME] += 1;
			}
		...
		showDropDownData("Lake", "1", "schools_and_principals", principalData);			// Show the school data
...
d3.json("../data/indiana_counties.json").then(function(indianaData) {
	...
	mapGroup.append("g")
			...
			.on("click", function(d){
				...
				showDropDownData(d.properties.NAME, d.properties.REGION_ID, "schools_and_principals", principalData);
...
function generatePDF()
{
	...
	doc.text(80, 10, 'Indiana School and Principal Data Report');
	...
	doc.setFontSize(8);
	var mapText = "The following dataset provides information on all the schools located in the " + 
	"state of Indiana. It also provides information on the school Principal and their contact information.";
	doc.text(10, 25, mapText, {'maxWidth' : maxWidth_2});

	// E. Map image:
	doc.setFontType("bold");
	var mapTitleText = "Schools and Principals in each county based on the 12 Indiana Economic Growth regions";
	doc.text(10, 35, mapTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[0], 'JPEG', 1, 40, 120 * mapWidth / mapHeight, 120 );
	

	// F. County bar graph:
	var countyBarGraphTitleText = "Distribution of schools in a specific Economic Growth Region";
	...
	// G. Pie Chart:
	var pieChartTitleText = "Percent distribution of total schools across Indiana's 12 EGR";
	...
	// H: Stats field:
	var statsTitleText = "Statistics for the number of schools present in each Economic Growth Region";
	...
	doc.setFontSize(8);
	var all92CountyBarGraphTitleText = "Comparison of the total schools in various counties spread in different regions over the entire state of Indiana.";
	...
	// Corporation Name:
	doc.text(10, baseDistance + textDistance + 5, "Corporation Name: " + dataForReport[1]);

	// Principal Name:
	doc.text(10, baseDistance + textDistance + 10, "School Name: " + dataForReport[2]);

	// Website:
	doc.text(10, baseDistance + textDistance + 15, "Principal Name: " + dataForReport[3]);

	// Address:
	doc.text(10, baseDistance + textDistance + 20, "Website: " + dataForReport[4]);

	// Project Summary:
	doc.text(10, baseDistance + textDistance + 25, "Address: " + dataForReport[5],
						  {'maxWidth' : maxWidth_2});

	
	doc.save("Schools and Principals Report.pdf")
