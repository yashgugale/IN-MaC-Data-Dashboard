...
// B. Global variables:
var adultAndWorkforceData = [];	// Store the principal data
...
var countyBarGraphProperties = {
	...
	chartPadding : 20,
	xLabel : "County Name",
	yLabel : "Adult and Workforce organization count",
...
var countyPieChartProperties = {
	...
	updatable : false,
	heightPadding : 5,
	tooltipData : "Total Adult and Workforce organizations in the region",
	tooltipHeight : "60px",
};
...
function init()
{
	...
	///////
	d3.csv("../data/06-adult_and_workforce_data.csv").then(function(data)
	{
		var i = 0;
		data.forEach(function(d){
			// trim() function is used to remove trailing and leading whitespaces from the data:
			adultAndWorkforceData.push([d.County.trim()]);								// Extract the County name
			adultAndWorkforceData[i].push(d.Organization.trim());						// Extract the Organization name
			adultAndWorkforceData[i].push(d["Contact Person"].trim());					// Extract the Contact name
			adultAndWorkforceData[i].push(d.Email.trim());								// Extract the Email ID
			var url = d.Website;
			...
			adultAndWorkforceData[i].push(url);										// Extract the webpage
			adultAndWorkforceData[i].push(d.Region.trim());							// Append the region ID as the last element
			i++;
		...
		showDropDownData("Lake", "1", "adultAndWorkforce", adultAndWorkforceData);			// Show the school data
...
d3.json("../data/indiana_counties.json").then(function(indianaData) {
	...
	mapGroup.append("g")
			...
			.on("click", function(d){
				...
				showDropDownData(d.properties.NAME, d.properties.REGION_ID, "adultAndWorkforce", adultAndWorkforceData);
...
function generatePDF()
{
	...
	doc.text(65, 10, 'Indiana Adult and Workforce Data Report');
	...
	doc.setFontSize(8);
	var mapText = "The following dataset provides information on all the workforce and employment related institutes located in the " + 
	"state of Indiana. It also provides information on the contact person and the website of the institute.";
	doc.text(10, 25, mapText, {'maxWidth' : maxWidth_2});

	// E. Map image:
	doc.setFontType("bold");
	var mapTitleText = "Adult and workforce development institutes in each county based on the 12 Indiana Economic Growth regions";
	doc.text(10, 35, mapTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[0], 'JPEG', 1, 40, 120 * mapWidth / mapHeight, 120 );
	

	// F. County bar graph:
	var countyBarGraphTitleText = "Distribution of the adult and workforce development institutes in a specific Economic Growth Region";
	...
	// G. Pie Chart:
	var pieChartTitleText = "Percent distribution of total adult and workforce development institutes across Indiana's 12 EGR";
	...
	// H: Stats field:
	var statsTitleText = "Statistics for the number of adult and workforce development institutes present in each Economic Growth Region";
	...
	doc.setFontSize(8);
	var all92CountyBarGraphTitleText = "Comparison of the total adult and workforce development institutes in various counties spread in different regions over the entire state of Indiana.";
	...
	doc.save("Adult and Workforce Development Report.pdf")
