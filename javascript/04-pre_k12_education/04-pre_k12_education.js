...
// B. Global variables:
var preK12Data = [];				// Store the principal data
...
var countyBarGraphProperties = {
	...
	chartPadding : 20,
	xLabel : "County Name",
	yLabel : "Pre K12 institute count",
...
var countyPieChartProperties = {
	...
	updatable : false,
	heightPadding : 5,
	tooltipData : "Total Pre K12 institutes in the region",
	tooltipHeight : "50px",
};
...
function init()
{
	...
	///////
	d3.csv("../data/04-pre_k12_data.csv").then(function(data)
	{
		var i = 0;
		data.forEach(function(d){
			// trim() function is used to remove trailing and leading whitespaces from the data:
			preK12Data.push([d.County.trim()]);								// Extract the County name
			preK12Data[i].push(d.Organization.trim());						// Extract the Organization name
			preK12Data[i].push(d["Contact Person"].trim());					// Extract the Contact name
			preK12Data[i].push(d.Email.trim());								// Extract the Email ID
			var url = d.Website;
			...
			preK12Data[i].push(url);										// Extract the webpage
			preK12Data[i].push(d.Region.trim());							// Append the region ID as the last element
			i++;
		...
		showDropDownData("Lake", "1", "preK12", preK12Data);			// Show the school data
...
d3.json("../data/indiana_counties.json").then(function(indianaData) {
	...
	mapGroup.append("g")
			...
			.on("click", function(d){
				...
				showDropDownData(d.properties.NAME, d.properties.REGION_ID, "preK12", preK12Data);
...
function generatePDF()
{
	...
	doc.text(80, 10, 'Indiana Pre K-12 Education Data Report');
	...
	doc.setFontSize(8);
	var mapText = "The following dataset provides information on all the Pre K-12 institutes located in the " + 
	...
	var mapTitleText = "Pre k-12 education institutes in each county based on the 12 Indiana Economic Growth regions";
	doc.text(10, 35, mapTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[0], 'JPEG', 1, 40, 120 * mapWidth / mapHeight, 120 );
	

	// F. County bar graph:
	var countyBarGraphTitleText = "Distribution of the pre k-12 institutes in a specific Economic Growth Region";
	...
	// G. Pie Chart:
	var pieChartTitleText = "Percent distribution of total pre k-12 institutes across Indiana's 12 EGR";
	...
	// H: Stats field:
	var statsTitleText = "Statistics for the number of pre k-12 institutes present in each Economic Growth Region";
	...
	doc.setFontSize(8);
	var all92CountyBarGraphTitleText = "Comparison of the total pre k-12 institutes in various counties spread in different regions over the entire state of Indiana.";
	...
	doc.save("Pre K-12 Education Report.pdf")
