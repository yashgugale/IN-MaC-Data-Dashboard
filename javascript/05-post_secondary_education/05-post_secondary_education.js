...
// B. Global variables:
var postSecondaryEducationData = [];	// Store the principal data
...
var countyBarGraphProperties = {
	...
	chartPadding : 20,
	xLabel : "County Name",
	yLabel : "Post Secondary Education institute count",
...
var countyPieChartProperties = {
	...
	updatable : false,
	heightPadding : 5,
	tooltipData : "Total Post Secondary institutes in the region",
	tooltipHeight : "60px",
};
...
function init()
{
	...
	///////
	d3.csv("../data/05-post_secondary_education_data.csv").then(function(data)
	{
		// console.log(data);
		var i = 0;
		data.forEach(function(d){
			// trim() function is used to remove trailing and leading whitespaces from the data:
			postSecondaryEducationData.push([d.County.trim()]);								// Extract the County name
			postSecondaryEducationData[i].push(d.Organization.trim());						// Extract the Organization name
			postSecondaryEducationData[i].push(d["Contact Person"].trim());					// Extract the Contact name
			postSecondaryEducationData[i].push(d.Email.trim());								// Extract the Email ID
			var url = d.Website;
			...
			postSecondaryEducationData[i].push(url);										// Extract the webpage
			postSecondaryEducationData[i].push(d.Region.trim());							// Append the region ID as the last element
			i++;
		...
		showDropDownData("Lake", "1", "postSecondary", postSecondaryEducationData);			// Show the school data
...
d3.json("../data/indiana_counties.json").then(function(indianaData) {
	...
	mapGroup.append("g")
			...
			.on("click", function(d){
				...
				showDropDownData(d.properties.NAME, d.properties.REGION_ID, "postSecondary", postSecondaryEducationData);
...
function generatePDF()
{
	...
	doc.text(65, 10, 'Indiana Post Secondary Education Data Report');
	...
	doc.setFontSize(8);
	var mapText = "The following dataset provides information on all the Post Secondary education institutes located in the " + 
	...
	var mapTitleText = "Post Secondary education institutes in each county based on the 12 Indiana Economic Growth regions";
	doc.text(10, 35, mapTitleText, {'maxWidth' : maxWidth_1});
	doc.addImage(all_imgs[0], 'JPEG', 1, 40, 120 * mapWidth / mapHeight, 120 );
	

	// F. County bar graph:
	var countyBarGraphTitleText = "Distribution of the post secondary education institutes in a specific Economic Growth Region";
	...
	// G. Pie Chart:
	var pieChartTitleText = "Percent distribution of total post secondary education institutes across Indiana's 12 EGR";
	...
	// H: Stats field:
	var statsTitleText = "Statistics for the number of post secondary education institutes present in each Economic Growth Region";
	...
	doc.setFontSize(8);
	var all92CountyBarGraphTitleText = "Comparison of the total post secondary education institutes in various counties spread in different regions over the entire state of Indiana.";
	...
	doc.save("Post Secondary Education Report.pdf")
